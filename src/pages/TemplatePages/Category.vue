  ...
  <div class="grid p-fluid animate__animated animate__fadeIn">
    <div class="col-12">
      <div class="card card-w-title">
        ...
        <DataTable  :value="templatedata" :rows="10" v-else :loading="loading.getData"
        ...
        <Dialog header="خطا" v-model:visible="display" :breakpoints="{ '960px': '75vw' }" :style="{ width: '30vw' }"
          ...
          <p class="line-height-3 m-0">
            این دسته بندی دارای {{ totalCountcategory }} تمپلیت می باشد و نمی توان آن را حذف کرد
          </p>
          <template #footer>
        ...
        <Dialog header="خطا" v-model:visible="displayStatus" :breakpoints="{ '960px': '75vw' }" :style="{ width: '30vw' }"
          :modal="true">
          <p class="line-height-3 m-0">
            این دسته بندی دارای {{ totalCountcategory }} تمپلیت می باشد و نمی توان آن را غیر فعال کرد
          </p>
          <template #footer>
...
import { templateCategoryService, templateService } from "../../@core/services/index";
...
export default {
  ...
  data() {
    return {
      ...
      templatestatus: [{ type: "همه" , value: null },
  ...
  methods: {
    ...
    async changeStatus(id) {
     ...
     await templateService.templateCategoryId({ ...this.filterCategory }).then(({ data }) => {
    ...
    editTemplate(data) {
      this.$router.push(`/categoryedit/${data.id}`)
